...
### Power BI
#### Import data to Power BI through Microsoft Exchange Online (Analytics email inbox). Importing most recent Salesforce report (attachment) in .csv format
...
#### Add Lapse Date column = Finish Date + 30 days, remove Finish Date column
...
#### Duplicate SF retention raw dataset twice raw. Rename tables: SF retention multi, SF retention unique 

Transformations for SF retention multi

* Keep duplicates of column Membership Purchase Name (MMS######)
...
#### Duplicate SF retention multi. Rename tables: SF retention multi (renewed/not renewed), SF retention multi (new/renewal)

Transformations for SF retention multi (renewed/not renewed) 

* Sort rows by Subscription History Name (MSA######) descending (this orders memberships from most recent subscription to oldest subscription)
* 
= Table.Sort(#"Kept Duplicates MMS",{{"Subscription History Name", Order.Descending}})


* Assign an index to all rows (each row is numbered from 0, at increments of 1)

= Table.AddIndexColumn(#"Sorted Subscriptions", "Index", 0, 1, Int64.Type)

* Group rows by Membership Purchase Name (MMS######)
...
* Assign an index to all rows (each row with matching MMS###### is numbered from 1, at increments of 1), name this index Subscription Number
* 
...
* Add column Renew vs. Not Renewed. Most recent subscription has Subscription Number = 1, (therefore it is the only one that has not yet been renewed)
...
Link ' SF retention'[Lapsed Date] to 'Date'[Date] in Model View.
...
|Membership  |Subscription History |Expiry    |New/Renewal  |Renewed/Not renewed |
| -----      | -----               | -----    | ----        | ---                |       
|MMS001234   |MSA33333             |01/2023   |Renewal			|Not renewed         |
|MMS001234   |MSA22222				     |01/2022		|Renewal			|Renewed             |
|MMS001234   |MSA11111     				 |01/2021		|New					|Renewed             |
|MMS005678	 |MSA44444				     |01/2023		|New					|Not renewed         |
